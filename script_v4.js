...
function changeMainPanel(url = null) {
    ...
    if (fileName === "究極の2択.html") {
        ...
        if (iframeElement) {
            ...
            // 同じURLならリロードしない
            if (lastScheduledFile === fileName || iframeElement.src.includes(fileName)) {
                lastScheduledFile = fileName;
                return;
            }
            const getBustedUrl = (u) => u + (u.includes('?') ? '&' : '?') + 't=' + Date.now();
            iframeElement.src = getBustedUrl(fileName);
        }
    }
    lastScheduledFile = fileName;
    ...
    if (data) {
        animateTransition(() => {
            if (data.mode === 'popup') {
                ...
                const getBustedUrl = (u) => u + (u.includes('?') ? '&' : '?') + 't=' + Date.now();
                iframe.src = getBustedUrl(data.url);
                iframe.onload = () => {
                    try {
...
function openPopupApp(data) {
    ...
    const getBustedUrl = (u) => u + (u.includes('?') ? '&' : '?') + 't=' + Date.now();
    document.getElementById('popup-iframe').src = getBustedUrl(data.url);
